...
const Stack = () => {   
    ...
    return (
        ...
        <div className="">
            ...
            {icondata.stack.icons.map(item => (
 <motion.div ref={ref} animate={animation} key={item.name} className="w-4/12 sm:w-3/12 md:w-2/12 lg:w-1/12 text-xs flex flex-col items-center justify-center p-4"><img src={item.img} alt=""/> <p className="mt-4 text-gray-700">{item.text}</p></motion.div>))
} 
        </div>
        </div>    
        </div>
